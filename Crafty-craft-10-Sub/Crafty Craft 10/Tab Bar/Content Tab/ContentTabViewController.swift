...
//        flushSearch()
    ...
    private func updateTabUI(selected: UIView, deselected: [UIView]) {
        selected.backgroundColor = #colorLiteral(red: 0, green: 0.6445246935, blue: 0.3786547482, alpha: 1)
...
//        cell.newIcon.isHidden = !cellModel.isContentNew
        ...
        var categoryImage: UIImage {
            switch tabsPageControllMode {
            case .skins:
                return UIImage(named: "skins_icon_button") ?? UIImage()
            case .addons:
                return UIImage(named: "addons_icon_button") ?? UIImage()
            case .maps:
                return UIImage(named: "maps_icon_button") ?? UIImage()
            }
        }
