...
class SkinChoicesVC: UIViewController {
    ...
    private func addBlurEffectToBackground() {
        ...
        applyBlurEffect = UIVisualEffectView(effect: blurEffect)
        applyBlurEffect?.frame = view.bounds
        applyBlurEffect?.alpha = 0
        view.addSubview(applyBlurEffect!)
        
        // Make the blur effect view cover the doneView
        view.bringSubviewToFront(containerView)
        
        UIView.animate(withDuration: 0.3) {
            self.applyBlurEffect?.alpha = 1
        }
    }
    
    private func configureTailoredButton(label: String) -> UIButton {
        let clicker = UIButton()
        clicker.frame = CGRectMake(0, 0, self.view.frame.width - buttonHorizontalOffset*2, buttonHeight)
        clicker.translatesAutoresizingMaskIntoConstraints = false
        clicker.heightAnchor.constraint(equalToConstant: buttonHeight).isActive = true
        clicker.setTitle(label, for: .normal)
        clicker.titleLabel?.font = UIFont.montserratFont(.bold, size: 18)
        clicker.setTitleColor(.black, for: .normal)
        clicker.backgroundColor = UIColor(named: "YellowSelectiveColor")
        clicker.cornerRadius = buttonHeight/2
        clicker.borderWidth = 1
        clicker.borderColor = .black
        return clicker
    }
