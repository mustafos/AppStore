...
protocol SkinHandlingPicker: AnyObject {
    func showPickSkin(for sub: AnatomyCreatedModel)
    func showEditPickerSkin(for sub: AnatomyCreatedModel)
}
...
class SkinCreatorMainVC: UIViewController {
    ...
    func pageLoop(_ m: Int, _ n: Int) -> [(Int, Int)] {
        var result: [(Int, Int)] = []
...
extension SkinCreatorMainVC: UICollectionViewDelegateFlowLayout {
    ...
    private func selectionNumSort<T: Comparable>(_ array: inout [T]) {
...
extension SkinCreatorMainVC: SearchableCollection {
    func filterData(with query: String?) {
        filterText = query
        menuCollectionView.reloadData()
    }
