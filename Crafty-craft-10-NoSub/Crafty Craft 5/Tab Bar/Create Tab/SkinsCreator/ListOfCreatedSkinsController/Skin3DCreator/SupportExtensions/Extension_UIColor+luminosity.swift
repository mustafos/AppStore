...
fileprivate extension CGFloat {
    ...
    func clampMinMaxVal(min: CGFloat, max: CGFloat) -> CGFloat {
        if self < min {
            return min
        } else if self > max {
            return max
...
//    static let background = UIColor(hex: 0x4C8BE)
    ...
    func withLuminosity(_ newLuminosity: CGFloat) -> UIColor {
        ...
        red = red.clampMinMaxVal(min: 0, max: 1)
        green = green.clampMinMaxVal(min: 0, max: 1)
        blue = blue.clampMinMaxVal(min: 0, max: 1)
