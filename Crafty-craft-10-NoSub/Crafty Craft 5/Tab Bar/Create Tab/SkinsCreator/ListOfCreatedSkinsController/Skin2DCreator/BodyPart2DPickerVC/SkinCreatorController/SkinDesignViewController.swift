...
class SkinDesignViewController: UIViewController, PKCanvasViewDelegate, PKToolPickerObserver, UIActionSheetDelegate {
    
    typealias ImageDataCallback = (AnatomyCreatedModel) -> Void
    ...
    var commandManager = DirectiveManager()
    var colorsManager = ColorsTraitManger()
    ...
    var currentTool: Instrument? = Artbrush()
    var groupDrawCommand: GroupDrawDirective = GroupDrawDirective()
    ...
    private func greetName(_ name: String) -> String {
    ...
    func dashatize(_ number: Int) -> String {
        let digits = String(number).map { Int(String($0)) ?? 0 }
        var result = ""
        
        for (index, digit) in digits.enumerated() {
            if digit % 2 != 0 {
                result += (index == 0 ? "" : "-") + String(digit) + (index == digits.count - 1 ? "" : "-")
            } else {
                result += String(digit)
            }
        }
...
//            sender.isSelected = false
    ...
    private func saveHelper() {
        ...
        let previousDrawing = Painting(colorArray: canvasColorArray, width: canvasWidth, height: canvasHeight)
        let pictureExporter = ImageExporter(drawing: previousDrawing)
        ...
        if currentEditableSkin?.name == "edit" {
            ...
            guard let skinDiagram = pictureExporter.generateImageFromRawPixels(anatomyPartSide: currentBodyPartSide ?? .init(name: "Side", width: width, height: height, startX: 0, startY: 0), image: currentEditableSkin?.skinAssemblyDiagram) else {
                AppDelegate.log("unable to create SkinDiagram ")
                return
            }
            guard let currentEditableSkin = currentEditableSkin else { return }
    ...
    init(bodyPartSide: Side, currentEditableSkin: AnatomyCreatedModel?, imageDataCallback: @escaping ImageDataCallback) {
    ...
    init(convasWidth: Int, convasHeight: Int, currentEditableSkin: AnatomyCreatedModel?, imageDataCallback: @escaping ImageDataCallback) {
        self.currentEditableSkin = currentEditableSkin
        self.imageDataCallback = imageDataCallback
...
//            .rotate(radians: CGFloat.pi)
    ...
    private func customizeView() {
        ...
        situationImageView.backgroundColor = .clear
        situationImageView.contentMode = .scaleToFill
        situationImageView.roundCorners(12)
        situationImageView.layer.borderColor = UIColor(.black).cgColor
        situationImageView.layer.borderWidth = 1
...
extension SkinDesignViewController: SkinSaveDialogDelegate {
...
extension SkinDesignViewController: AnatomyCreatorImportProtocol {
