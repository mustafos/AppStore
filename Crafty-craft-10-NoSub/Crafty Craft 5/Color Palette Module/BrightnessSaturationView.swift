...
internal class BrightnessSaturationView: UIView {
    ...
    private func createSelector(_ selectorDiameter: CGFloat, _ scale: CGFloat) {
        ...
        selector.roundCorners(selectorDiameter / 2)
        selector.layer.borderColor = UIColor.white.cgColor
        selector.layer.borderWidth = 3
        selector.isUserInteractionEnabled = true
        
        tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(tappedOnView))
        dragGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(tappedOnView))
